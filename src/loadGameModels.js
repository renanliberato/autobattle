...
export async function loadGameModels() {
    ...
    const healthbarModel = await loadModelAsync('./src/3dmodels/healthbar.glb');
    healthbarModel.scale.set(0.5, 0.3, 0.5);
    threeDModels.healthbar.add(healthbarModel);

    threeDModels.tile = await loadModelAsync('./src/3dmodels/tile.glb');

    threeDModels.sword = new THREE.Group();
    threeDModels.sword.add(await loadModelAsync('./src/3dmodels/sword.glb'));
    ...
    threeDModels.dagger.add(await loadModelAsync('./src/3dmodels/dagger.glb'));
    ...
    threeDModels.scepter.add(await loadModelAsync('./src/3dmodels/scepter.glb'));
    ...
    threeDModels.orbred.add(await loadModelAsync('./src/3dmodels/orbred.glb'));
    threeDModels.orbred.children[0].scale.set(0.15, 0.15, 0.15);

    threeDModels.bow = new THREE.Group();
    threeDModels.bow.add(await loadModelAsync('./src/3dmodels/bow.glb'));
    ...
    threeDModels.shield.add(await loadModelAsync('./src/3dmodels/shield.glb'));
    threeDModels.shield.children[0].scale.set(0.5, 0.5, 0.5);
    threeDModels.shield.children[0].rotation.y = -0.25 * Math.PI * 2;

    threeDModels.tower = await loadModelAsync('./src/3dmodels/tower.glb');
    threeDModels.tower.scale.set(0.5, 0.5, 0.5);

    threeDModels.cloud = await loadModelAsync('./src/3dmodels/cloud.glb');

    threeDModels.arrow = new THREE.Group();
    threeDModels.arrow.add(await loadModelAsync('./src/3dmodels/arrow.glb'));
    ...
    threeDModels.tileinventory.add(await loadModelAsync('./src/3dmodels/tileinventory.glb'));
}
